...
export default function FeedbackButton({ triggerContent }: FeedbackProps) {
  const [open, setOpen] = React.useState(false);

  const { isMobile } = useMediaQuery();

  const description =
    "Want me to make a video?  Leave a suggestions for me below.";
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="sm:max-w-[425px]">
        <DialogHeader>
          <DialogTitle className="text-red-400 mb-4">
            Video Suggestions
          </DialogTitle>
